...
export type DownloadStatus = 'pending' | 'downloading' | 'transcoding' | 'failed' | 'finished' | 'completed' | 'separating' | 'dereverbing' | 'transcribing';
 ...
 * @param customUrl - Optionale custom URL aus Settings
 * @param fallbackUrl - Fallback URL wenn keine custom URL gesetzt ist
 * @returns Promise<string> - QR-Code als Data URL
 */
export const generateQRCodeDataUrl = async (
  customUrl?: string, 
  fallbackUrl: string = 'http://localhost:5000/new'
): Promise<string | null> => {
  try {
    let qrUrl: string;
